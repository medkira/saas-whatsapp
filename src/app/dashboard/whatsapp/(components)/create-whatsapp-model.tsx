...
import { linkWhatsapp } from '@/actions/whatsapp';

export default function CreateWhatsAppModel() {
  ...
  return (
    <div className="flex flex-col gap-2">
      <Button className="max-w-fit font-bold" color="success" onPress={onOpen}>
        Link Whatsapp
      </Button>
...
function Form({ onClose }: { onClose: any }) {
  ...
  const [state, dispatch] = useFormState(linkWhatsapp, initialState);
      ...
      /> */}
        ...
        >
          {categories.map((category) => (
            <AutocompleteItem key={category}>{category}</AutocompleteItem>
          ))}
        </Autocomplete>
        <Input
          label="Reference"
